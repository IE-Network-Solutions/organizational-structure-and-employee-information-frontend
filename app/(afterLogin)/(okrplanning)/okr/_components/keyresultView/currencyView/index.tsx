...
const CurrencyView: React.FC<OKRProps> = ({
  keyValue,
  index,
  isEdit,
  form,
  ...
  const viewPrefix = keyValue?.id
    ? `okr-currency-view-${keyValue.id}`
    : `okr-currency-view-${index}`;
  return (
    <div
          ...
          <div className="flex-1">
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: 'Currency title is required',
                  validator: (notused, value) =>
                    value && value.trim() !== ''
                      ? Promise.resolve()
                      : Promise.reject(new Error('Currency title is required')),
                },
              ]}
            >
              <Input
