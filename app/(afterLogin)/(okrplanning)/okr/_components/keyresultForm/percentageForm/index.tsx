...
const PercentageForm: React.FC<OKRFormProps> = ({
  ...
  return (
    ...
    >
      <div className="absolute top-2 right-2" style={{ zIndex: 10 }} data-cy={`okr-percentage-remove-wrapper-${index}`}>
        <KeyResultRemoveButton
      ...
      >
          ...
          <>
            <div id={`okr-percentage-desktop-top-row-${index}`} data-cy={`okr-percentage-desktop-top-row-${index}`} className="flex flex-row gap-2 items-center mt-4 mx-4">
              ...
              <Form.Item className="w-24 mb-0" name="weight" rules={[{ required: true, message: 'Please enter the weight as a Percentage' }]} id={`key-result-weight-${index}`} data-cy={`okr-percentage-desktop-weight-item-${index}`}>
                <InputNumber className="w-full h-10 rounded-lg text-base" data-cy={`okr-percentage-desktop-weight-input-${index}`} min={0} max={100} suffix="%" value={keyItem.weight} onChange={(value) => updateKeyResult(index, 'weight', value)} disabled={disableWeightEdit} aria-label="Weight" />
              </Form.Item>
        ...
        {!isMobile && !isBasic && (
          <>
            {isCardView ? (
              ...
              >
                <div className={`${ADVANCED_ROW_CLASS} items-end`} data-cy={`okr-percentage-desktop-advanced-row-${index}`}>
                  <Form.Item
                  ...
                  <Form.Item className="mb-0" label={<span className="opacity-0 select-none" data-cy={`okr-percentage-save-label-spacer-${index}`}>Save</span>}>
                    <Button
                      type="primary"
        ...
        {isMobile && (
          <>
            {isCardView ? (
              <div className="mt-4 mx-4" data-cy={`okr-percentage-mobile-card-wrapper-${index}`}>
                <div
                ...
                >
                  <div className="flex items-start justify-between" data-cy={`okr-percentage-mobile-saved-card-header-${index}`}>
                    <div className="flex flex-col gap-2 flex-1 min-w-0" data-cy={`okr-percentage-mobile-saved-card-content-${index}`}>
                      <span className="text-xs font-medium text-gray-600 border border-gray-300 rounded-md px-2.5 py-1.5 w-fit inline-block" data-cy={`okr-percentage-mobile-saved-card-weight-${index}`}>
                        Weight {keyItem.weight ?? 0}%
                      </span>
                      <p className="text-sm font-semibold text-gray-900 truncate" data-cy={`okr-percentage-mobile-saved-card-title-${index}`}>
                        {keyItem.title ? keyItem.title : <span className="text-gray-400 italic" data-cy={`okr-percentage-mobile-saved-card-untitled-${index}`}>Untitled key result</span>}
                      </p>
                    </div>
                    <Tooltip title="Edit">
                      <button
                  ...
                  <div className="flex flex-row flex-wrap gap-2 pt-1 border-t border-gray-100" data-cy={`okr-percentage-mobile-saved-card-values-${index}`}>
                    <span className="text-xs font-medium text-gray-600 border border-gray-300 rounded-md px-2.5 py-1.5 w-fit inline-block" data-cy={`okr-percentage-mobile-saved-card-initial-label-${index}`}>
                      Initial Value : <span className="font-semibold text-gray-900" data-cy={`okr-percentage-mobile-saved-card-initial-value-${index}`}>{Number(keyItem.initialValue ?? 0).toLocaleString()}%</span>
                    </span>
                    <span className="text-xs font-medium text-gray-600 border border-gray-300 rounded-md px-2.5 py-1.5 w-fit inline-block" data-cy={`okr-percentage-mobile-saved-card-target-label-${index}`}>
                      Target Value : <span className="font-semibold text-gray-900" data-cy={`okr-percentage-mobile-saved-card-target-value-${index}`}>{Number(keyItem.targetValue ?? 0).toLocaleString()}%</span>
                    </span>
                  </div>
                </div>
              </div>
            ) : (
              <div id={`okr-percentage-mobile-wrapper-${index}`} data-cy={`okr-percentage-mobile-wrapper-${index}`} className="space-y-4 mt-4 mx-4">
                <Form.Item
                  className="mb-0"
                  name="title"
                  label={<KeyResultFieldLabel label="Key Result" tooltip={KEY_RESULT_TOOLTIP} />}
                ...
                <div id={`okr-percentage-mobile-values-row-${index}`} data-cy={`okr-percentage-mobile-values-row-${index}`} className="flex gap-4">
                  <Form.Item
                    className="flex-1 mb-0"
                    name="initialValue"
                    label={<KeyResultFieldLabel label="Initial Value" tooltip="Initial percentage value (0-100)" />}
                    rules={[{ required: true, message: 'Please enter the initial value' }]}
                  ...
                  <Form.Item
                    className="flex-1 mb-0"
                    name="targetValue"
                    label={<KeyResultFieldLabel label="Target Value" tooltip="Target percentage value (0-100)" />}
                    rules={[{ required: true, message: 'Please enter the target value' }]}
                ...
                <div id={`okr-percentage-mobile-meta-row-${index}`} data-cy={`okr-percentage-mobile-meta-row-${index}`} className="flex gap-2 items-end">
                  <Form.Item
                    className="flex-1 mb-0"
                    name="weight"
                    label={<KeyResultFieldLabel label="Weight" tooltip={WEIGHT_TOOLTIP} />}
