  ...
  return (
    ...
    >
      ...
      >
          ...
          <>
            <div id={`okr-currency-desktop-top-row-${index}`} data-cy={`okr-currency-desktop-top-row-${index}`} className="flex flex-row gap-2 items-center mt-4 mx-4">
              ...
              <Form.Item className="w-24 mb-0" name={`weight_${index}`} rules={[{ required: true, message: 'Please enter the weight' }]} id={`weight-input-${index}`} data-cy={`okr-currency-desktop-weight-item-${index}`}>
                <InputNumber className="w-full h-10 rounded-lg text-base" data-cy={`okr-currency-desktop-weight-input-${index}`} min={0} max={100} suffix="%" placeholder="100" value={keyItem.weight} onChange={(value) => updateKeyResult(index, 'weight', value)} aria-label="Weight" />
              </Form.Item>
        ...
        <div id={`okr-currency-mobile-wrapper-${index}`} data-cy={`okr-currency-mobile-wrapper-${index}`} className={`${isMobile ? 'block' : 'hidden'} space-y-4 mt-4 mx-4`}>
          <Form.Item
            className="mb-0"
            name={`key_name_${index}`}
            label={<KeyResultFieldLabel label="Key Result" tooltip={KEY_RESULT_TOOLTIP} />}
          ...
          <div id={`okr-currency-mobile-meta-row-${index}`} data-cy={`okr-currency-mobile-meta-row-${index}`} className="flex gap-2">
            <Form.Item
              className="flex-1 mb-0"
              name={`weight_${index}`}
              label={<KeyResultFieldLabel label="Weight" tooltip={WEIGHT_TOOLTIP} />}
          ...
          <div id={`okr-currency-mobile-values-row-${index}`} data-cy={`okr-currency-mobile-values-row-${index}`} className="flex gap-4 pl-4">
            <Form.Item className="flex-1 mb-0" name="initialValue" rules={[{ required: true, message: 'Please enter the initial value' }]} data-cy={`okr-currency-mobile-initial-item-${index}`}>
              <InputNumber className="w-full h-10 rounded-lg text-base" data-cy={`okr-currency-mobile-initial-input-${index}`} min={0} placeholder="Initial Value" addonAfter={<span className="text-base" data-cy={`okr-currency-mobile-initial-currency-${index}`}>$</span>} value={keyItem.initialValue === 0 ? undefined : keyItem.initialValue} onChange={(value) => updateKeyResult(index, 'initialValue', value)} onKeyPress={(e) => { if (!/[0-9]/.test(e.key) && e.key !== 'Backspace' && e.key !== 'Delete' && e.key !== 'Tab' && e.key !== '.') { e.preventDefault(); } }} />
            </Form.Item>
            <Form.Item className="flex-1 mb-0" name="targetValue" rules={[{ required: true, message: 'Please enter the target value' }]} data-cy={`okr-currency-mobile-target-item-${index}`}>
              <InputNumber className="w-full h-10 rounded-lg text-base" data-cy={`okr-currency-mobile-target-input-${index}`} min={0} placeholder="Target Value" addonAfter={<span className="text-base" data-cy={`okr-currency-mobile-target-currency-${index}`}>$</span>} value={keyItem.targetValue === 0 ? undefined : keyItem.targetValue} onChange={(value) => updateKeyResult(index, 'targetValue', value)} onKeyPress={(e) => { if (!/[0-9]/.test(e.key) && e.key !== 'Backspace' && e.key !== 'Delete' && e.key !== 'Tab' && e.key !== '.') { e.preventDefault(); } }} />
            </Form.Item>
          </div>
        </div>
      </Form>
