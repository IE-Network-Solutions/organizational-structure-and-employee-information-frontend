...
const KeyResultTableRow: FC<KeyResultTableRowProps> = ({
  ...
  return (
    <>
      ...
      >
        <td
          className="px-6 py-4 text-sm text-gray-900 font-normal whitespace-normal break-normal min-w-[280px] align-top"
        ...
        <td
          className="px-6 py-4 whitespace-nowrap"
          data-cy={`okr-key-result-table-row-indicator-${keyResult?.id}`}
        >
          <div
            className="flex items-center space-x-1"
            data-cy={`okr-key-result-table-row-indicator-wrap-${keyResult?.id}`}
          >
            ...
            {keyResult?.metricType?.name !== 'Milestone' && (
              <div
                className="w-2 h-2 rounded-full bg-okr-primary"
                data-cy={`okr-key-result-table-row-progress-dot-${keyResult?.id}`}
              />
            )}
          </div>
        </td>
        <td
          className="px-6 py-4 whitespace-nowrap min-w-[180px]"
          data-cy={`okr-key-result-table-row-status-${keyResult?.id}`}
        >
          <div
          ...
          >
            {isBasicAchieveOrNot ? (
              ...
              >
                <div
                  className="w-[140px] bg-gray-200 rounded-full h-2 overflow-hidden"
              ...
              >
                <div
                  className="w-[140px] bg-gray-200 rounded-full h-2 mr-3 overflow-hidden"
                  data-cy={`okr-key-result-table-row-progress-bar-bg-${keyResult?.id}`}
                >
                  <div
