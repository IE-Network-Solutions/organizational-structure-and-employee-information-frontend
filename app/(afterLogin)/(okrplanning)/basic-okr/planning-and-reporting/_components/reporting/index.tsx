...
export default function BasicReporting() {
    ...
    const { data: allUserPlanning } = useGetUserPlanning(planningPeriodId ?? '', activePlanPeriod.toString());
    ...
    const activeTabName = planningPeriods?.items?.find((p: any) => p.id === planningPeriodId)?.name || 'Report';

    const reportSummaries = useMemo(() => {
        return reportingData?.items?.map((dataItem: any) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <h2 className="text-lg font-bold text-gray-800">{activeTabName} Reporting</h2>
                <Tooltip title={!allUserPlanning || allUserPlanning.length === 0 ? 'Please create plan first' : ''}>
                    <CustomButton
                    ...
                    reportSummaries.map((plan: any) => (
                        <BasicPlanCard
                            ...
                            planStatus={{
                                ...
                                date: plan.reportedDate ? dayjs(plan.reportedDate).format('MMMM Do YYYY, h:mm:ss A') : (plan.createdAt ? dayjs(plan.createdAt).format('MMMM Do YYYY, h:mm:ss A') : 'N/A'),
