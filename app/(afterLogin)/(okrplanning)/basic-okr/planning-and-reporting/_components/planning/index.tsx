...
import {
  Cadence,
  ViewMode,
} from '@/app/(afterLogin)/(planningAndReporting)/planning-and-reporting/_components/types';
...
export default function BasicPlanning() {
  ...
  const [planningPeriodType, setPlanningPeriodType] = useState<
    'monthly' | 'weekly' | 'daily'
  >('weekly');
  const [isDailyModalOpen, setIsDailyModalOpen] = useState(false);
  const [isWeeklyMonthlyModalOpen, setIsWeeklyMonthlyModalOpen] =
    useState(false);
  ...
  // mutations
  const { mutate: createPlanTasks, isLoading: isCreating } =
    useCreatePlanTasks();
  ...
  const { mutate: createReport, isLoading: isReporting } =
    useCreateBasicReport();
  ...
  const weeklyPeriod = userPlanningPeriods?.find(
    (p: any) => p.planningPeriod?.name?.toLowerCase() === 'weekly',
  );
  const dailyPeriod = userPlanningPeriods?.find(
    (p: any) => p.planningPeriod?.name?.toLowerCase() === 'daily',
  );
  const monthlyPeriod = userPlanningPeriods?.find(
    (p: any) => p.planningPeriod?.name?.toLowerCase() === 'monthly',
  );
  ...
  const { data: weeklyPlanningData, isLoading: isLoadingWeekly } =
    useGetPlanning({
      userId: selectedUser,
      ...
      pageSize: 100,
      sessionId:
        selectedSessionIds.length > 0 ? selectedSessionIds : allSessionsOfYear,
    });
  ...
  const { data: userDailyPlans } = useGetUserPlanning(
    dailyPeriod?.planningPeriod?.id || '',
    '1',
  );
  const { data: userWeeklyPlans } = useGetUserPlanning(
    weeklyPeriod?.planningPeriod?.id || '',
    '2',
  );
  const { data: userMonthlyPlans } = useGetUserPlanning(
    monthlyPeriod?.planningPeriod?.id || '',
    '3',
  );

  const { data: dailyHierarchy } = useGetPlanningPeriodsHierarchy(
    userId,
    dailyPeriod?.planningPeriod?.id || '',
  );
  const { data: weeklyHierarchy } = useGetPlanningPeriodsHierarchy(
    userId,
    weeklyPeriod?.planningPeriod?.id || '',
  );
  ...
  const isDailyDisabled = useMemo(() => {
    ...
    const noParentPlan =
      dailyHierarchy?.parentPlan &&
      ((dailyHierarchy.parentPlan.plans?.length ?? 0) === 0 ||
        (dailyHierarchy.parentPlan.plans?.filter((p: any) => !p.isReported)
          .length ?? 0) === 0);
    return (
      hasUnreportedPlan || noParentPlan || (objective?.items?.length ?? 0) === 0
    );
  ...
  const isWeeklyDisabled = useMemo(() => {
    ...
    const noParentPlan =
      weeklyHierarchy?.parentPlan &&
      ((weeklyHierarchy.parentPlan.plans?.length ?? 0) === 0 ||
        (weeklyHierarchy.parentPlan.plans?.filter((p: any) => !p.isReported)
          .length ?? 0) === 0);
    return (
      hasUnreportedPlan || noParentPlan || (objective?.items?.length ?? 0) === 0
    );
  ...
  const handlePlanTypeChange = (value: string) => {
    ...
    if (value === 'all') {
      ...
      const subordinates =
        employeeData?.items
          ?.filter(
            (e: any) => (e.delegatedTo?.id || e.reportingTo?.id) === userId,
          )
          .map((e: any) => e.id) || [];
      setSelectedUser(
      ...
      if (selectedPlanType === 'all') {
        ...
        const subordinates =
          employeeData?.items
            ?.filter(
              (e: any) =>
                (e.delegatedTo?.id || e.reportingTo?.id) === userId &&
                departmentUserIds.includes(e.id),
            )
            .map((e: any) => e.id) || [];
        setSelectedUser(
          subordinates.length > 0
  ...
  const combinedPlanningData = useMemo(() => {
    const weeklyItems =
      weeklyPlanningData?.items?.map((item: any) => ({
        ...item,
        isWeekly: true,
      })) ?? [];
    const dailyItems =
      dailyPlanningData?.items?.map((item: any) => ({
        ...item,
        isWeekly: false,
      })) ?? [];
  ...
  const transformedData =
    groupPlanTasksByKeyResultAndMilestone(combinedPlanningData);
  ...
  const planSummaries = useMemo(() => {
    return (
      transformedData?.map((dataItem: any) => {
        const rawItem = combinedPlanningData?.find(
          (i: any) => i.id === dataItem.id,
        );
        const cadence = (rawItem?.isWeekly ? 'weekly' : 'daily') as Cadence;

        const report = combinedReportingData.find(
          (r: any) => r.planId === dataItem.id,
        );
        const isReported =
          rawItem?.isReported ||
          !!rawItem?.report ||
          (Array.isArray(rawItem?.reports) && rawItem.reports.length > 0) ||
          !!report;
          ...
          summary.tasks = summary.tasks.map((t: any) => {
            ...
            const status =
              rawStatus !== undefined && rawStatus !== null && rawStatus !== ''
                ...
                : rawTask?.isAchieved === true
                  ? 'completed'
                  : rawTask?.isAchieved === false
                    ? 'failed'
                    : t.status || 'pending';
            return { ...t, status };
          });
        ...
        const comments =
          isReported && report ? report.comments || [] : summary.comments || [];
        const commentCount =
          isReported && report
        ...
        return {
          ...
          reportedDate:
            rawItem?.reportedDate ||
            rawItem?.report?.createdAt ||
            report?.createdAt,
          comments,
          commentCount,
        };
    ...
    transformedData,
    employeeData,
    combinedPlanningData,
    combinedReportingData,
  ]);

  const handleTaskStatusChange = (taskId: string, newStatus: string) => {
    const plan = combinedPlanningData.find((p) =>
  ...
  const handleEdit = (planSummary: any) => {
    const rawPlan = combinedPlanningData.find(
      (p: any) => p.id === planSummary.id,
    );
    if (!rawPlan) return;
    setIsEdit(true);
    const cadence = (rawPlan.isWeekly ? 'weekly' : 'daily') as
      | 'weekly'
      | 'daily'
      | 'monthly';
  ...
  const handleAddDailyPlan = (values: any) => {
    const allWeeklyTasks =
      weeklyPlanningData?.items?.flatMap((p: any) => p.tasks || []) || [];

    const findParentPlan = (parentId: string) => {
      return weeklyPlanningData?.items?.find((p: any) =>
        p.tasks?.some((t: any) => t.id === parentId),
      );
    };

    if (isEdit) {
      const updatedTasks = values.tasks.map((task: any) => {
        const parentTask = allWeeklyTasks.find(
        ...
        return {
          ...
          keyResultId:
            parentTask?.keyResultId || parentTask?.keyResult?.id
              ? String(parentTask.keyResultId || parentTask.keyResult.id)
              : null,
          milestoneId:
            parentTask?.milestoneId || parentTask?.milestone?.id
              ? String(parentTask.milestoneId || parentTask.milestone.id)
              : null,
      ...
      createPlanTasks(
        {
          tasks: values.tasks.map((task: any) => {
            ...
            return {
              ...
              keyResultId:
                parentTask?.keyResultId || parentTask?.keyResult?.id || null,
              milestoneId:
                parentTask?.milestoneId || parentTask?.milestone?.id || null,
            };
          }),
        },
  ...
  const handleAddWeeklyMonthlyPlan = (values: any) => {
    const periodId =
      planningPeriodType === 'weekly'
        ? weeklyPeriod?.planningPeriod?.id
        : monthlyPeriod?.planningPeriod?.id;
    const planningUserId =
      planningPeriodType === 'weekly'
        ? weeklyPlanningUserId
        : monthlyPlanningUserId;
    ...
    if (isEdit) {
      ...
      updatePlanTasks({ tasks: updatedTasks });
      setIsWeeklyMonthlyModalOpen(false);
      setIsEdit(false);
      setEditingData(null);
    } else {
      createPlanTasks(
      ...
      items.push({
        ...
        label: (
          <Tooltip
            title={
              isMonthlyDisabled
                ? userMonthlyPlans?.length > 0
                  ? 'Report planned tasks before you create Monthly plan'
          ...
          >
            <span
              style={
                isMonthlyDisabled
                  ? { color: '#8F94A3', cursor: 'not-allowed', opacity: 0.6 }
                  : {}
              }
              data-cy="monthly-plan-menu-item"
            >
              Monthly
            </span>
          </Tooltip>
        ...
        onClick: () => {
          setPlanningPeriodType('monthly');
          setIsEdit(false);
          setIsWeeklyMonthlyModalOpen(true);
        },
      });
    }
      ...
      items.push({
        ...
        label: (
          <Tooltip
            title={
              isWeeklyDisabled
                ? userWeeklyPlans?.length > 0
                  ? 'Report planned tasks before you create Weekly plan'
                  : weeklyHierarchy?.parentPlan
                    ? `Please create ${weeklyHierarchy.parentPlan.name} Plan before creating Weekly Plan`
                    : 'Create Objective before you Plan'
                : ''
            }
          >
            ...
            title={
              isDailyDisabled
                ? userDailyPlans?.length > 0
                  ? 'Report planned tasks before you create Daily plan'
                  : dailyHierarchy?.parentPlan
                    ? `Please create ${dailyHierarchy.parentPlan.name} Plan before creating Daily Plan`
                    : 'Create Objective before you Plan'
                : ''
            }
          >
  ...
  const handleSaveReport = (values: any) => {
    ...
    values.tasks.forEach((formTask: any) => {
      if (formTask.isAchieved !== null) {
        ...
        const originalTask = reportingPlan?.tasks?.find(
          (t: any) => t.id === formTask.id,
        );
        const targetValue =
          originalTask?.target || originalTask?.targetValue || 100;
    ...
    const planPeriodId =
      reportingPlan.cadence === 'weekly'
        ? weeklyPeriod?.planningPeriod?.id
        : reportingPlan.cadence === 'daily'
          ? dailyPeriod?.planningPeriod?.id
          : monthlyPeriod?.planningPeriod?.id;
    ...
    const cadenceLabel = reportingPlan.cadence
      ? String(reportingPlan.cadence).charAt(0).toUpperCase() +
        String(reportingPlan.cadence).slice(1)
      : 'Report';
    const reportTitle = `${reportingPlan?.title ?? 'Plan'} - ${cadenceLabel} Report`;
    const reportScore =
      totalCount > 0
        ? String(Math.round((achievedCount / totalCount) * 100))
        : '0';
  ...
  return (
    <div className="space-y-6" data-cy="planning-container">
      ...
      <PlanFilters
        ...
        selectedEmployee={
          selectedUser?.[0] === 'all' ? 'all' : selectedUser?.[0]
        }
        ...
        {isLoadingLoad ? (
          ...
          planSummaries.map((plan: any) => (
            <BasicPlanCard
              ...
              planStatus={{
                label:
                  plan.status.label === 'Open' ? 'Pending' : plan.status.label,
                date:
                  plan.status.label === 'Closed'
                    ? dayjs(plan.status.updatedAt).format(
                        'MMMM D YYYY, h:mm:ss A',
                      )
                    : dayjs(plan.createdAt).format('MMMM D YYYY, h:mm:ss A'),
                status:
                  plan.status.label === 'Closed'
                    ? ('success' as any)
                    : 'pending',
              ...
              canApprove={
                String(userId) ===
                String(
                  getEmployeeData(plan.owner?.id)?.delegatedTo?.id ||
                    getEmployeeData(plan.owner?.id)?.reportingTo?.id,
                )
              }
              canEdit={
                String(userId) === String(plan.owner?.id) &&
                (plan.status?.label === 'Open' ||
                  plan.status?.label === 'Pending') &&
                !plan.isReported &&
          ...
          <div
            className="bg-white p-12 rounded-xl border border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-500"
            data-cy="no-plans-empty-state"
          >
            <Empty description="No plans found for this period" />
          </div>
        )}
      </div>
      ...
      <div
        ...
        data-cy="planning-pagination-container"
      >
        <CustomPagination
          current={page}
          total={combinedPlanningData?.length || 0}
          pageSize={pageSize}
      ...
      <DailyPlanModal
        ...
        weeklyPlans={
          weeklyPlanningData?.items
            ?.filter((p: any) => !p.isReported)
            ?.map((p: any) => ({
              id: p.id,
