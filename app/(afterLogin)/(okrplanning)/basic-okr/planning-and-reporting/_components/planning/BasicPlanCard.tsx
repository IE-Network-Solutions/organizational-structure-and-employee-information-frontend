...
export default function BasicPlanCard({
  ...
  if (!isExpanded) {
    return (
      <div
        className="bg-white rounded-xl p-5 border border-gray-200 flex items-center justify-between cursor-pointer hover:shadow-md transition-all"
        data-cy="basic-plan-card-collapsed"
      >
        <h3
          className="text-blue-600 font-semibold text-lg"
          data-cy="basic-plan-card-title-collapsed"
        >
          {title}
        </h3>
      </div>
  ...
  return (
    <div
      ...
      data-cy="basic-plan-card"
    >
      {/* Header */}
      <div
        className="flex justify-between items-start mb-2"
        data-cy="basic-plan-card-header"
      >
        <h3
          className="text-blue-600 font-bold text-lg"
          data-cy="basic-plan-card-title"
        >
          {title}
        </h3>
          ...
          >
            <div className="flex items-center gap-1.5" data-cy="planned-date">
              <span
                ...
                data-cy="planned-label"
              >
                Planned
              </span>
              <span
                className="font-bold text-[#161A2C]"
                data-cy="planned-value"
              >
                {date}
              </span>
            </div>
            {isReported && reportedDate && (
              <div
                ...
                data-cy="reported-date"
              >
                <span
                  className="text-gray-400 font-normal"
                  data-cy="reported-label"
                >
                  Reported
                </span>
                <span
                  className="font-bold text-[#161A2C]"
                  data-cy="reported-value"
                >
                  {reportedDate}
                </span>
              </div>
            ...
            >
              ...
              >
                ...
                <div className="hidden md:flex flex-col min-w-0" data-cy="status-info">
                  ...
                  <p
                    className="text-[#718096] text-xs mt-0.5"
                    data-cy="status-date"
                  >
                    {planStatus.date}
                  </p>
      ...
      {/* Tasks List */}
      <div className="space-y-3 mb-6" data-cy="tasks-list">
        {tasks.map((task) => (
          <TaskRow
            key={task.id}
            task={task}
      ...
      <div className="mt-4" data-cy="comments-section">
        <CommentsSection
