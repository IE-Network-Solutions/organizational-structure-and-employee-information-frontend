...
const OneToOne: React.FC<StatisticsCardProps> = ({ data }) => {
  ...
  return (
      ...
      <Card className="p-4 flex flex-col items-center shadow-lg rounded-lg text-center">
        <Link href={`/feedback/conversation/one-on-one/${543211234}`} passHref><h3 className="text-lg font-semibold mb-2">{title}</h3></Link>
        <Link href={`/feedback/conversation/one-on-one/${543211234}/questions`} passHref>       
        ...
        <div className="text-sm text-gray-700 space-y-1">
          ...
          <Link href={`/feedback/conversation/one-on-one/${543211234}/meetings`} passHref>
