...
const ActionPlans: React.FC<PropsData> = ({slug}:PropsData) => {
  const {data:conversationInstanceActionPlan}=useGetAllActionPlansByConversationInstanceId(slug);
  ...
  return (
    <div>
      {conversationInstanceActionPlan?.items?.map((actionPlan:any, index:number) => (
        <Card
          key={actionPlan.id}
          title={
            ...
            <List.Item>
              <div className="flex w-full items-start">
                ...
                <div className="w-1/3 flex flex-col space-y-4">
                  <div className="text-gray-500 font-semibold">Responsible Person</div>
                ...
                <div className="w-2/3 flex flex-col space-y-4">
                    ...
                    {actionPlan?.assigneeId ? (
                      <div className="flex items-center">
                        ...
                        <span className="ml-2 font-semibold">
                          {getEmployeeData(actionPlan?.assigneeId)?.firstName ?? ''}{' '}
                          {getEmployeeData(actionPlan?.assigneeId)?.lastName ?? ''}
                        </span>
                      </div>
                    ) : (
                  ...
                  <div>
                    {actionPlan?.comment ? (
                      <span className="text-gray-700">{actionPlan?.comment}</span>
                    ) : (
