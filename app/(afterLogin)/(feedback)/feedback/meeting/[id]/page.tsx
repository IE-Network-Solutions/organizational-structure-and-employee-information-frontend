...
export default function MeetingDetailPage({ params }: { params: Params }) {
  ...
  return (
    <div className="p-1 " data-cy={`feedback-meeting-id-page-div-${params?.id}`} id={`feedback-meeting-id-page-div-${params?.id}`}>
      <MeetingHeader
        loading={isLoading}
        title={meeting?.title}
        meetingData={meeting}
      ...
      />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2" data-cy={`feedback-meeting-id-page-div-grid-${params?.id}`} id={`feedback-meeting-id-page-div-grid-${params?.id}`}>
        <div className="lg:col-span-2 border rounded-lg h-screen overflow-y-auto scrollbar-none" data-cy={`feedback-meeting-id-page-div-main-content-${params?.id}`} id={`feedback-meeting-id-page-div-main-content-${params?.id}`}>
          <MeetingObjectives
            loading={isLoading}
            objective={meeting?.objective}
            data-cy="feedback-meeting-id-page-meeting-objectives"
          />
          <MeetingAgenda canEdit={canEdit} id={meeting?.id} data-cy="feedback-meeting-id-page-meeting-agenda" />
          <FinalNotes
            loading={isLoading}
        ...
        <div className="border rounded-lg  h-screen overflow-y-auto scrollbar-none" data-cy={`feedback-meeting-id-page-div-sidebar-${params?.id}`} id={`feedback-meeting-id-page-div-sidebar-${params?.id}`}>
          <OtherDetails
            canEdit={canEdit}
            loading={isLoading}
            meeting={meeting}
          ...
          />
          <PreviousMeeting meeting={meeting} data-cy="feedback-meeting-id-page-previous-meeting" />
        </div>
      </div>
    </div>
  );
