...
const EmployeeName = ({
    ...
    const userName =
        ...
        '-';
    return (
        <Text style={style}>{userName || 'Unknown User'}</Text>
    );
};
...
const EmployeeRole = ({ userId, style }: { userId: string, style?: any }) => {
    ...
    return <Text style={style}>{userDetails?.employeeJobInformation[0]?.position?.name || '-'}</Text>
}
...
const MomTemplate = ({ meetingData }: { meetingData: any }) => {
    const meetingDuration = meetingData?.startAt && meetingData?.endAt ? dayjs.duration(dayjs(meetingData.endAt).diff(dayjs(meetingData.startAt))).humanize() : 'N/A';
    const location = `${meetingData?.physicalLocation ?? ''} ${meetingData?.virtualLink ? `(${meetingData.virtualLink})` : ''}`.trim();
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.attendeesSection}>
                    ...
                    <View style={styles.table}>
                        <View style={styles.tableRow}>
                            <Text style={[styles.tableColHeader, styles.tableColHeaderStart]}>Attendees</Text>
                            <Text style={styles.tableColHeader}>Role</Text>
                            <Text style={[styles.tableColHeader, styles.tableColHeaderEnd]}>Attendance</Text>
                        </View>
                        {meetingData?.attendees?.map((attendee: any, index: number) => (
                            <View style={styles.tableRow} key={index}>
                                ...
                                <View style={styles.tableCol}>
                                    ...
                                    {attendee.attendanceStatus === 'late' && attendee.lateBy > 0 && (
                                        <Text style={styles.subtleText}>{attendee.lateBy} min</Text>
                                    )}
                                    {attendee.attendanceStatus === 'absent' && attendee.absentismReason && (
                                        <Text style={styles.subtleText}>Reason: {attendee.absentismReason}</Text>
                                    )}
                                </View>
                            </View>
                        ))}
                ...
                <View style={styles.agendaSection}>
                    ...
                    {meetingData?.agenda?.map((item: any, index: number) => (
                        <View style={styles.agendaItem} key={index}>
                            <Text style={styles.agendaTitle}>{`${index + 1}. ${item.agenda}`}</Text>
                        </View>
                    ))}
                </View>
                ...
                {meetingData?.finalNote && (
                    <View style={styles.finalNoteSection}>
                        <Text style={[styles.sectionTitle, { textAlign: 'left' }]}>Discussion notes</Text>
                ...
                <View style={styles.actionPlanSection}>
                    ...
                    <View style={styles.table}>
                        <View style={styles.tableRow}>
                            <Text style={[styles.actionPlanTableColHeader, styles.actionPlanTableColHeaderStart]}>Action Item</Text>
                            <Text style={styles.actionPlanTableColHeader}>
                                Responsible Person
                            </Text>
                            <Text style={styles.actionPlanTableColHeader}>Priority</Text>
                            <Text style={[styles.actionPlanTableColHeader, styles.actionPlanTableColHeaderEnd]}>Deadline</Text>
                        </View>
                            ...
                            <View style={styles.tableRow} key={index}>
                                ...
                                <View style={styles.actionPlanTableCol}>
                                    {plan.responsibleUsers.map((user: any) => (
                                        <EmployeeName key={user.responsibleId} userId={user.responsibleId} style={{ fontSize: 10 }} />
                                    ))}
                                </View>
                                <Text style={[
                                    styles.actionPlanTableCol,
                                    plan.priority?.toLowerCase() === 'high' ? styles.priorityHigh : {},
                                    plan.priority?.toLowerCase() === 'medium' ? styles.priorityMedium : {},
                                    plan.priority?.toLowerCase() === 'low' ? styles.priorityLow : {},
                                ]}>
                                    {plan.priority}
                                </Text>
                                <Text style={styles.actionPlanTableCol}>{dayjs(plan.deadline).format('MMM DD, YYYY')}</Text>
                            </View>
                        ))}
                    </View>
                </View>
