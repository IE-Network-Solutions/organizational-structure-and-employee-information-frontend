...
function FeedbackTypeDetail({ feedbackTypeDetail }: FeedbackTypeDetailProps) {
  ...
  const {setVariantType,selectedFeedback,setOpen,open,setSelectedFeedback } = ConversationStore();
              ...
              <Card className='mx-2' key={item.id}>
                <div className='flex justify-between'>
                    ...
                    <p className='flex gap-2'>
                      <Button size='small' onClick={()=>editHandler(item)} icon={<Edit2Icon className='w-4 h-4 text-xs'/>} type='primary' />
                      <Popconfirm
                          ...
                          cancelText="No"
                        >
                          <Button size="small" loading={deleteLoading} icon={<MdDeleteOutline />} danger type="primary" />
                      </Popconfirm> 
                     </p>
                 </div>
                  </Card>
