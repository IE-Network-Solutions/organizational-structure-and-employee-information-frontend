...
const TaskCard: React.FC = () => {
    ...
    const { isLoading, data: userInfo } = useGetEmployee(userId);
    ...
    const handleDeleted = (itemIndex: number, taskIndex: number) => {
      ...
      // Extract isEdit while keeping the rest of the properties
      const { isEdit, ...filteredData } = {
        ...newData[itemIndex].tasks[taskIndex],
      };
      console.log(filteredData, 'deletedData');
    ...
    const getDepartmentData = (id: string) => {
      const depDetail = Departments?.find(
        (dep: any) => dep?.id === id,
      );
