...
import { useGetDepartmentChild, useGetWeeklyPriorities, useGetWeeks } from '@/store/server/features/okrplanning/weeklyPriority/queries';
import TaskCard from '../taskCard';

const Department: React.FC = () => {
  const { addNewCard, data, setData,departmentId,setDepartmentId,setWeekIds,weekIds,activeTab } = useWeeklyPriorityStore();
  ...
  const departmentIds = Array.isArray(departmentChild) ? departmentChild.map((item) => item.id) : [];
  const departIds = (!departmentIds?.length || activeTab === 2) 
  ...
  const { data: weeks} =
    useGetWeeks();
  console.log(departmentChild, 'weeklyPriority');
  // const datas = groupByDepartment(weeklyPriority?.items);
  useEffect(() => {
    ...
    <div style={{ padding: 20 }}>
      <div className="flex justify-between mb-5">
        <div className="flex gap-4">
          <Select
            id={`selectDepartment`}
            placeholder={activeTab==1?"Search and select a department":"Search and select a team"}
