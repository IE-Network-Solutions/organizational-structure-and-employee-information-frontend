...
function Page() {
  ...
  const {
    data: settlementTrackingData,
    isLoading,
    refetch,
  } = useGetSettlementTracking(searchParams);
  const { data: compensationData, isLoading: compensationLoading } =
    useGetAllowance();
  const { data: employeeData, isLoading: employeeLoading } = useGetAllUsers();
  const { mutate: deleteSettlementTracking, isLoading: deleteLoading } =
    useDeleteSettlementTrackingByEmployeeId();
  ...
  const totalEmployee = new Set(
    settlementTrackingData?.map(
      (item: any) => item.compensationItemEntitlement.employeeId,
    ),
  ).size;

  const totalCompensation = new Set(
    settlementTrackingData?.map((item: any) => item.compensationItemId),
  ).size;

  const totalPaidAmount =
    settlementTrackingData?.reduce(
      (acc: any, item: any) => (item.isPaid ? acc + Number(item.amount) : acc),
      0,
    ) ?? 0;
  ...
  const dataSource = Object.entries(grouped).map(([key, value]) => ({
    employeeId: key,
    payPeriodId: Array.isArray(value)
      ? value.map((item: any) => item.payPeriodId)
      : [],
    amount: Array.isArray(value)
      ? value.reduce((acc, item) => acc + (Number(item.amount) || 0), 0)
      : 0,
    totalPaid: Array.isArray(value)
      ? value
          .filter((item: any) => item.isPaid)
          ?.reduce((acc: any, item: any) => acc + (Number(item.amount) || 0), 0)
      : 0,
    compensationIds: Array.isArray(value)
      ? value.map((item: any) => item.compensationItemId)
      : [],
    compensationNames: Array.isArray(value)
      ? value.map((item) => item.compensationItemEntitlement.name)
      : [],
  }));
  ...
  const getCompensationName = (compensationId: any) => {
    const compensation = compensationData?.find(
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (payPeriodIds: any) => {
        ...
        const startDates = periods.map((p: any) =>
