...
import { useGetAllPayrollApprovals } from '@/store/server/features/payroll/payrollApproval/queries';
import {
...
const Payroll = () => {
  ...
  const payPeriodIdFromRoute = String(params?.payPeriodId || '');
  ...
  const [payPeriodQuery, setPayPeriodQuery] = useState(
    payPeriodIdFromRoute ? `?payPeriodId=${payPeriodIdFromRoute}` : '',
  );
  const [payPeriodId, setPayPeriodId] = useState(payPeriodIdFromRoute);
  ...
  const payrollExists =
    (payroll?.meta?.totalItems ?? payroll?.items?.length ?? 0) > 0;
  const {
    showApproveButton,
    pendingApproval,
    refetchPendingApprovals,
    refetchPayrollApprovalForPeriod,
  } = usePayrollApproveVisibility(currentPayPeriodId, payrollExists);

  const { data: allPayrollApprovals } = useGetAllPayrollApprovals();

  const payrollApprovalForPeriod = useMemo(() => {
    return (Array.isArray(allPayrollApprovals) ? allPayrollApprovals : []).find(
      (item: { payPeriodId?: string }) =>
        item.payPeriodId === currentPayPeriodId,
    );
  }, [allPayrollApprovals, currentPayPeriodId]);

  const isPayrollApproved = payrollApprovalForPeriod?.approved === true;
  ...
  }, [isPayrollApproved, currentPayPeriodId]);
  ...
  useEffect(() => {
    if (!payPeriodIdFromRoute) return;
    setPayPeriodId(payPeriodIdFromRoute);
    setPayPeriodQuery(`?payPeriodId=${payPeriodIdFromRoute}`);
    setSearchValue((prev) =>
      mergePayrollFilterState(prev, { payPeriodId: payPeriodIdFromRoute }),
    );
    const existing = useEmployeeStore.getState().searchQuery || '';
    const paramsFromQuery = new URLSearchParams(
      existing.startsWith('&') || existing.startsWith('?')
        ? existing.slice(1)
        : existing,
    );
    paramsFromQuery.set('payPeriodId', payPeriodIdFromRoute);
    setSearchQuery(`&${paramsFromQuery.toString()}`);
  }, [payPeriodIdFromRoute, setSearchQuery]);
    ...
    if (bankLetter) {
      if (!isPayrollApproved) {
        notification.error({
          ...
          description:
            'Bank letter can only be exported after payroll has been approved.',
        });
        if (!exportPayrollData && !paySlip && !exportBank) {
          return;
        }
      } else {
  ...
  const handleSearch = (partial: {
    ...
    if (merged.payPeriodId) {
      ...
      if (merged.payPeriodId !== payPeriodIdFromRoute) {
        router.push(`/payroll/${merged.payPeriodId}/payroll`);
      }
    }
  ...
  const handleApprovePayroll = () => {
    ...
    approvePayroll(approvalData, {
      onSuccess: (data) => {
        if (data?.last === true) {
          lastApproving(currentPayPeriodId, { onSuccess: handleSuccess });
        } else {
          handleSuccess();
        }
      },
  ...
  const selectedPayPeriod = (payPeriodData || []).find(
    (period: { id: string }) => period.id === currentPayPeriodId,
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            ...
            className="w-full"
          >
            <div className="hidden" data-cy="payroll-tab-legacy-header">
              <CustomBreadcrumb
                href="/payroll"
                backControlDataCy="payroll-detail-back-link"
                title={
                  <span
                    ...
                    items={[
                      {
                        title: (
                          <Link
                            href="/payroll"
                            data-cy="payroll-breadcrumb-payroll-link"
                      ...
                      },
                      {
                        title: (
                          <Link
                            href="/payroll"
                            data-cy="payroll-breadcrumb-pay-periods-link"
                            className="text-xs sm:text-sm"
                          >
                            Pay Periods
                          </Link>
      ...
      >
        ...
        >
          {/* Toolbar Section ΓÇö Search + Filter */}
          <div
          ...
          >
            ...
            <div
              className="flex items-center gap-2"
              data-cy="payroll-filters-actions"
            >
              <FilterPopover
              ...
              />
              <Button
                id="payroll-export-open-modal-click-button"
                data-cy="payroll-export-open-modal-click-button"
              ...
              >
                <span
                  ...
                  data-cy="payroll-export-button-label"
                >
                  Export
                </span>
              </Button>
          ...
          {/* Pagination footer ΓÇö outside scrollable table wrapper */}
          <div
            ...
            >
              ...
              {!isPayrollApproved && (
                <Text
                  ...
                  data-cy="payroll-export-bank-letter-approval-hint"
                >
                  Available after payroll is approved.
                </Text>
              )}
              <Switch
