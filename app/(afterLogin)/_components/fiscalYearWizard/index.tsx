...
export default function FiscalYearWizard({
    ...
    try {
      ...
      if (hasOverlap) {
        form.setFields([
          {
            name: 'startDate',
            errors: ['Date overlaps with existing fiscal year'],
          },
          {
            name: 'endDate',
            errors: ['Date overlaps with existing fiscal year'],
  ...
  const handleGoToMonths = () => {
    ...
    sessions.forEach((s, idx) => {
      ...
      if (
        sStart.isBefore(fyStart, 'day') ||
        sEnd.isAfter(fyEnd, 'day') ||
        sStart.isAfter(sEnd)
      ) {
      ...
      if (idx > 0) {
        ...
        if (
          !sStart.isAfter(prevEnd, 'day') &&
          !sStart.isSame(prevEnd.add(1, 'day'), 'day')
        ) {
          newErrors[`session-dates-${idx}`] = true;
          hasError = true;
        }
  ...
  const handleOnFinish = () => {
    ...
    sessions.forEach((s, sIdx) => {
      ...
      s.months.forEach((m, mIdx) => {
        ...
        if (
          mStart.isBefore(sStart, 'day') ||
          mEnd.isAfter(sEnd, 'day') ||
          mStart.isAfter(mEnd)
        ) {
        ...
        if (mIdx > 0) {
          ...
          if (
            !mStart.isAfter(prevEnd, 'day') &&
            !mStart.isSame(prevEnd.add(1, 'day'), 'day')
          ) {
            newErrors[`month-dates-${sIdx}-${mIdx}`] = true;
            hasError = true;
          }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <CloseOutlined
            className="absolute right-6 text-gray-400 cursor-pointer hover:text-gray-600"
            onClick={onClose}
          />
        </div>
                    ...
                    >
                      ...
                      >
                        <DatePicker.RangePicker
                          ...
                          onChange={(dates) =>
                            handleSessionDateChange(sIndex, dates)
                          }
                          variant="borderless"
                          suffixIcon={null}
                          allowClear={false}
                        ...
                        >
                          ...
                          >
                            <DatePicker.RangePicker
                              ...
                              onChange={(dates) =>
                                handleMonthDateChange(sIndex, mIndex, dates)
                              }
                              variant="borderless"
                              allowClear={false}
                              suffixIcon={
              ...
              {sessions.map((session, index) => (
                ...
                >
                  <Input
                    value={session.name}
                    onChange={(e) =>
                      handleSessionNameChange(index, e.target.value)
                    }
                  ...
                  >
                    <DatePicker.RangePicker
                      ...
                      onChange={(dates) =>
                        handleSessionDateChange(index, dates)
                      }
                      variant="borderless"
                      allowClear={false}
                      suffixIcon={
  ...
  return (
    ...
    >
      ...
      >
        ...
        <CloseOutlined
          className="absolute right-4 top-5 text-gray-400 cursor-pointer hover:text-gray-600"
          onClick={onClose}
        />
      </div>
        ...
        >
          ...
          >
            ...
            >
              <Radio
                value={12}
                className={`w-full border rounded-lg p-3 transition-all m-0 items-start ${
                  breakdown === 12
