...
const GenerateModal: React.FC = () => {
  ...
  return (
    <Modal
      ...
      title={<div id="generate-modal-title" data-cy="generate-modal-title" className="font-semibold text-md">Generate Incentive</div>}
      open={showGenerateModal}
      onCancel={handleModalClose}
      centered
      closable={false}
      footer={
        <div id="generate-modal-footer" data-cy="generate-modal-footer" className="flex items-center justify-center gap-3 mt-2">
          <Button
    ...
    >
      ...
      >
        <Form.Item
          ...
          label={
            <div id="generate-modal-form-generate-all-label" data-cy="generate-modal-form-generate-all-label" className="text-medium font-md">
          ...
          name="generateAll"
        >
          <Switch id="generate-modal-form-generate-all-switch" data-cy="generate-modal-form-generate-all-switch" onChange={handleSwitchChange} />
        </Form.Item>
        ...
        <Form.Item
          ...
          label={
            <span id="generate-modal-form-session-label" data-cy="generate-modal-form-session-label" className="font-semibold">
              Select Session<span id="generate-modal-form-session-required" data-cy="generate-modal-form-session-required" className="text-red-500">*</span>
            </span>
          }
          name="session"
        ...
        >
          {isSwitchOn ? (
            ...
            <Select id="generate-modal-form-session-select" data-cy="generate-modal-form-session-select" mode="multiple" size="large" placeholder="Select session">
              {allSessions?.items?.map((session: any) => (
                <Select.Option id={`generate-modal-form-session-option-${session?.id}`} data-cy={`generate-modal-form-session-option-${session?.id}`} value={session?.id} key={session?.id}>
                  {session?.name}
                </Select.Option>
              ))}
            </Select>
        ...
        <Form.Item
          ...
          label={
            <span id="generate-modal-form-pay-period-label" data-cy="generate-modal-form-pay-period-label" className="font-semibold">
              Pay Period<span id="generate-modal-form-pay-period-required" data-cy="generate-modal-form-pay-period-required" className="text-red-500">*</span>
            </span>
          }
          className="mb-4"
        ...
        >
          ...
          >
            {payPeriodData?.map((payPeriod: any) => (
              <Select.Option id={`generate-modal-form-pay-period-option-${payPeriod?.id}`} data-cy={`generate-modal-form-pay-period-option-${payPeriod?.id}`} value={payPeriod?.id} key={payPeriod?.id}>
