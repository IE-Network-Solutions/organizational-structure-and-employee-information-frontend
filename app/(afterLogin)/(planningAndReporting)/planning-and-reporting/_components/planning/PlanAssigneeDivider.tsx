...
export default function PlanAssigneeDivider({
  ...
  return (
    ...
    >
      <div className="h-px min-w-0 flex-1 bg-[#E5E7EB]" aria-hidden />
      <span className="shrink-0 text-xs font-medium text-[#8F94A3]">
        {name}
        {role && role !== 'N/A' && role !== 'Plan' ? (
          <span className="font-normal text-[#B0B3C0]"> · {role}</span>
        ) : null}
      </span>
      <div className="h-px min-w-0 flex-1 bg-[#E5E7EB]" aria-hidden />
    </div>
