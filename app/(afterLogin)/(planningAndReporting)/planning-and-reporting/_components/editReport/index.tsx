              ...
              keyresult?.metricType?.name !== NAME.MILESTONE && (
                  ...
                  rules={[
                    {
                      validator(nonused, value) {
                        ...
                        if (isNot && numericValue > task?.targetValue) {
                          return Promise.reject(
                            new Error(
                              `Value shouldn't exceed ${Number(task?.targetValue)?.toLocaleString()}`,
                            ),
                          );
                        }
                ...
                >
                  <InputNumber
                    ...
                    controls={false}
                    onChange={(value) => {
                      const statusValue = form.getFieldValue([
                        task.taskId,
                        'status',
                      ]);
                      if (statusValue === 'Done') {
            ...
            >
              <div className="flex items-center gap-3">
                  ...
                  onClick={() => {
                    ...
                    form.setFieldsValue({
                      [task.taskId]: {
                        status: 'Done',
                        actualValue:
                          Number(task?.targetValue ?? 0)?.toLocaleString(),
                      },
                    });
                  }}
                >
                  ...
                  <span
                    className={`text-sm ${isDone ? 'text-gray-900 font-medium' : 'text-gray-500'}`}
                  >
                    Done
                  </span>
                </div>
                ...
                >
                  ...
                  <span
                    className={`text-sm ${isNot ? 'text-gray-900 font-medium' : 'text-gray-500'}`}
                  >
                    Not
                  </span>
                </div>
