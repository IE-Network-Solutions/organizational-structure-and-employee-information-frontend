...
export const groupUnReportedTasksByKeyResultAndMilestone = (reportTasks: any) => {
  console.log(reportTasks,"reportTasks")
  const keyResultMap = reportTasks?.reduce((acc: any, task: any) => {
    ...
    const taskObject = {
      ...task.planTask, // Include all attributes of planTask
      taskId: task?.id,
      taskName: task?.task,
  ...
  const objectiveMap:any = keyResultArray.reduce((acc: any, keyResult: any) => {
