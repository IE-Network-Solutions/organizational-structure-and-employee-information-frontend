...
import { useGetActiveTrainingCommitmentsByUser } from '@/store/server/features/tna/trainingCommitment/queries';
import CommitmentProgressBar from '@/app/(afterLogin)/(tna)/tna/_components/commitmentProgressBar';

/**
 * Employee-facing tracker for their own active training commitments. Renders
 * nothing when the employee has none, so the grid stays the focus.
 */
const MyCommitmentsPanel: FC = () => {
  const router = useRouter();
  const { userId } = useAuthenticationStore();
  const { data: commitments, isLoading } =
    useGetActiveTrainingCommitmentsByUser(userId ?? '');
      ...
      >
        {commitments.map((commitment) => (
          <button
            ...
            onClick={() =>
              router.push(
                `/tna/management/external/${commitment.externalTrainingRequestId}`,
              )
            }
          ...
          >
            ...
            >
              ...
              >
                {commitment.courseName || 'External training'}
              </span>
              <span
