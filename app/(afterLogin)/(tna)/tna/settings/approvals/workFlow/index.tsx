...
/**
 * Timesheet lets one workflow cover several request types, so it renders a
 * checkbox group here. TNA has a single request type, so the type is fixed and
 * the chooser is dropped rather than shown with one permanently-ticked box.
 */
const TNA_REQUEST_TYPE_LABEL = 'Training Needs Assessment';
  ...
  const handleCreate = async () => {
    try {
      ...
      const steps = selections.SectionItemType.flatMap((selection, idx) => {
        ...
        // Levels left blank must not become `{ userId: undefined }` steps —
        // the approval service rejects those with "steps.N.userId must be a
        // string" and the whole workflow fails to save.
      ...
      if (!steps.length) {
        NotificationMessage.error({
          message: 'Error',
          description:
            'Add at least one approver before creating the workflow.',
        });
        return;
      }
          ...
          >
            ...
            >
              <div data-cy="approval-workflow-final-info">
                ...
                >
                  Request type: {TNA_REQUEST_TYPE_LABEL}
                </span>
              </div>
              <span
                className="inline-flex items-center rounded-lg border border-gray-200 bg-[#f7f7f7] px-2 py-0.5 text-sm text-[#4d4d4d]"
