...
import { Form, Input, DatePicker, Radio, Button, FormInstance } from 'antd';
...
export default function FiscalYearForm({ form }: { form: FormInstance }) {
  ...
  const generateSessions = (start: dayjs.Dayjs, end: dayjs.Dayjs, breakdown: number) => {
        ...
        <Form
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Form.Item
              ...
              name="startDate"
              rules={[{ required: true, message: 'Select fiscal year start date' }]}
            >
              <DatePicker onChange={() => breakdown !== null && onBreakdownChange(breakdown)} className="w-full h-10" />
            </Form.Item>

            <Form.Item
              label="Fiscal Year End Date"
              name="endDate"
              rules={[{ required: true, message: 'Select fiscal year end date' }]}
            >
              <DatePicker onChange={() => breakdown !== null && onBreakdownChange(breakdown)} className="w-full h-10" />
            </Form.Item>
          </div>
          ...
          <Form.Item
            ...
            name="breakdown"
            rules={[{ required: true, message: 'Please select a fiscal period breakdown' }]}
          >
            <Radio.Group
            ...
            >
              <Radio value={4} className="rounded-md border p-4">Quarterly</Radio>
              <Radio value={2} className="rounded-md border p-4">Semester</Radio>
              <Radio value={1} className="rounded-md border p-4">Yearly</Radio>
            </Radio.Group>
          </Form.Item>
