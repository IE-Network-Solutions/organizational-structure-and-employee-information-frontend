...
interface EmployeeTodaysAttendanceCardProps {
...
export default function EmployeeTodaysAttendanceCard({
  ...
  lateCount = 25,
  absentCount = 25,
}: EmployeeTodaysAttendanceCardProps) {
  ...
  const monthChips = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  const yearChips = Array.from({ length: 2026 - 2016 + 1 }, (_, i) => String(2016 + i));
    ...
    >
      ...
      >
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isActive ? 'bg-gray-100 text-gray-900' : 'bg-white text-gray-500 hover:bg-gray-50',
                ].join(' ')}
        ...
        className={[
          ...
          chipsAnim === 'out' ? 'opacity-0 translate-y-1' : 'opacity-100 translate-y-0',
        ].join(' ')}
      ...
      >
        <div className="flex items-center gap-2" id="legend-on-time" data-cy="legend-on-time">
          <MdFiberManualRecord size={12} className="text-green-500" />
          <span className="text-black/50 font-medium" data-cy="legend-on-time-text">
            On Time {onTimeCount}
          </span>

        </div>
        <div className="flex items-center gap-2" id="legend-late" data-cy="legend-late">
          <MdFiberManualRecord size={12} className="text-orange-500" />
          <span className="text-black/50 font-medium" data-cy="legend-late-text">
            Late {lateCount}
          </span>
        </div>
        <div className="flex items-center gap-2" id="legend-absent" data-cy="legend-absent">
          <MdFiberManualRecord size={12} className="text-red-500" />
          <span className="text-black/50 font-medium" data-cy="legend-absent-text">
            Absent {absentCount}
          </span>
        </div>
      </div>
