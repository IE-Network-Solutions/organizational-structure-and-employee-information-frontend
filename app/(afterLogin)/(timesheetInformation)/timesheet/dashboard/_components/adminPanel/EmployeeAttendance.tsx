...
import { Table, Input, Select, DatePicker, Pagination, Avatar, Tag } from "antd";
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-sm">
      ...
      <Table
        ...
        onRow={(record) => ({
          onClick: () => {
            router.push(`/timesheet/dashboard?employeeAttendance&user=${record.key}`);
          },
      ...
      <div className="flex justify-between items-center mt-4 text-sm text-gray-600">
        <Pagination
          ...
          className="flex"
        />
        <div>{filteredData.length} Result{filteredData.length !== 1 ? "s" : ""}</div>
      </div>
    </div>
  );
}
