...
const LeaveSectionGraph: React.FC = () => {
    const { setDepartmentOnLeave, departmentOnLeave, startDate, endDate, setStartDate, setEndDate } = TimeAndAttendaceDashboardStore();
    const { data: employeeAdminLeave, isLoading: loading } = useGetAdminOnLeave({ userId: '', startDate: startDate, endDate: endDate, departmentId: departmentOnLeave });
        ...
        <div className="col-span-7">
            ...
            <Spin spinning={loading}>
                {employeeAdminLeave?.monthlyStats?.length === 0 ?
                    ...
                    <Line className='h-64' data={lineChartData} options={lineChartOptions} />
                }
            </Spin>
        </div>
