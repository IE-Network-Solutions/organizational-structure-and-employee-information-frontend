...
const FIELD_TYPES = [
  { id: 'short_text', type: FieldType.SHORT_TEXT, label: 'Text Field', description: 'Input field for text' },
  { id: 'paragraph', type: FieldType.PARAGRAPH, label: 'Text Area', description: 'Input field for larger text' },
  { id: 'checkbox', type: FieldType.CHECKBOX, label: 'Checkbox', description: 'Input field for multiple values' },
  { id: 'multiple_choice', type: FieldType.MULTIPLE_CHOICE, label: 'Radio box', description: 'Input field for single value' },
  { id: 'dropdown', type: FieldType.DROPDOWN, label: 'Dropdown', description: 'Input field for selecting a value' },
];
...
const ApplicationFormDragDrop: React.FC<ApplicationFormDragDropProps> = ({ form }) => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === SOURCES_DROPPABLE && destination.droppableId === CANVAS_DROPPABLE) {
      const fieldTypeInfo = FIELD_TYPES.find((f) => f.id === result.draggableId);
      if (fieldTypeInfo) {
    ...
    if (source.droppableId === CANVAS_DROPPABLE && destination.droppableId === CANVAS_DROPPABLE) {
  ...
  const getTypeLabel = (type: string) => FIELD_TYPES.find((f) => f.type === type)?.label ?? 'Field';
    ...
    >
      <Form
        ...
        >
          ...
          >
            <Input placeholder="Input" className="h-10 rounded-lg" data-cy="talent-acquisition-custom-field-name" />
          </Form.Item>
          <Form.Item
            ...
            rules={[{ required: true, message: 'Please select a field validation type!' }]}
          >
            <Select
          ...
          <Form.Item name="isActive" valuePropName="checked">
            <Checkbox data-cy="talent-acquisition-custom-field-active">
              ...
              <span className="ml-1 text-gray-500">If the field is active will show</span>
            </Checkbox>
          </Form.Item>
          <Form.Item name="required" valuePropName="checked">
            <Checkbox data-cy="talent-acquisition-custom-field-required">
              <span className="text-gray-700">Required</span>
              <span className="ml-1 text-gray-500">If selected it must be filled</span>
            </Checkbox>
          </Form.Item>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Droppable droppableId={SOURCES_DROPPABLE} isDropDisabled={true}>
          {(provided) => (
            ...
            >
              <span className="text-sm font-semibold text-gray-700">Available field types</span>
          ...
          {(provided, snapshot) => (
            <div
              ...
              className={`rounded-lg border border-dashed p-3 min-h-[200px] transition-colors ${
                snapshot.isDraggingOver ? 'border-[#6366F1] bg-[#6366F1]/5' : 'border-gray-200 bg-white'
              }`}
            >
              <span className="text-sm font-semibold text-gray-700">Application form</span>
              <p className="mt-1 text-xs text-gray-500">Drag fields here to add them to the form.</p>
              <div className="mt-3 space-y-2">
                {questions.map((q, index) => (
                  <Draggable key={q.id || index} draggableId={q.id || String(index)} index={index}>
                    {(provided, snapshot) => (
                      <div
                        ref={provided.innerRef}
                        {...provided.draggableProps}
                        ...
                        className={`flex items-center gap-2 rounded-lg border bg-white p-3 cursor-grab active:cursor-grabbing ${
                          snapshot.isDragging ? 'opacity-90 shadow-lg ring-2 ring-[#6366F1] cursor-grabbing' : 'border-gray-200'
                        }`}
                      >
