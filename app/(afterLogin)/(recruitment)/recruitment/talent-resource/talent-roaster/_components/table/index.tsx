...
import { Col, Row, Select, Table, TableColumnsType ,DatePicker, Input, Modal, Button, Dropdown} from "antd";  
...
interface TalentRoasterTableProps {
  onEdit?: (data: any) => void;
    ...
    const TalentRoasterTable = ({ onEdit }: TalentRoasterTableProps) => {
        ...
        const { data: talentRoaster, isLoading: isTalentRoasterLoading } = useGetTalentRoaster({
        ...
        const { mutate: deleteTalentRoaster, isLoading: isDeleteLoading } = useDeleteTalentRoaster();
    ...
    const columns: TableColumnsType<any> = [
      ...
      const data = talentRoaster?.items?.map((item: any, index: any) => {
  ...
  onRow={(record) => ({
    onClick: (event) => {
      ...
      const isInteractiveElement = target.closest('input[type="checkbox"], button, a, .ant-btn, .ant-checkbox');
      
      if (!isInteractiveElement) {
        router.push(`/recruitment/talent-resource/talent-roaster/${record?.id}`);
      }
    }
  })}
    />
