...
function scoreFromActual(kpi: ScorecardKpiRow): number | null {
  ...
  if (target <= 0) return Math.min(Math.max(Math.round(kpi.actual), 0), kpi.weight);
      ...
      <div className="px-4 pt-4 pb-2 sm:px-6 sm:pt-6 flex items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center px-2.5 py-1 rounded text-xs font-medium bg-[#DBEAFE] text-blue-700 border border-[#BFDBFE] whitespace-nowrap">
              {progressPct}% KPI Progress
            </span>
            <span className="hidden sm:inline-flex items-center px-2.5 py-1 rounded text-xs font-medium border border-gray-200 text-gray-600 bg-white whitespace-nowrap">
              {daysLeft == null ? '—' : daysLeft} Days Left
            </span>
          </div>
          <h2 className="mt-3 mb-0 text-base sm:text-lg font-bold text-gray-900 leading-7 sm:leading-8">
            {title}
          </h2>
        </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200 text-sm">
            {kpis.length === 0 ? (
              ...
              kpis.map((kpi) => {
                ...
                const score =
                  drafts[key] !== undefined ? drafts[key] : scoreFromActual(kpi);
                const barPct = reportingOpen
                  ...
                  : kpi.progress;
                const editable =
                  reportingOpen && isRowEditable(kpi, scorecard);
                return (
                  <tr
                    key={kpi.id}
                    ...
                    <td className="px-3 py-3 whitespace-nowrap sm:px-6 sm:py-4">
                      {barPct >= 100 ? (
                        ...
                        <div className="flex min-w-0 items-center">
                          <div className="mr-3 h-2 w-[140px] overflow-hidden rounded-full bg-gray-200">
                            <div
