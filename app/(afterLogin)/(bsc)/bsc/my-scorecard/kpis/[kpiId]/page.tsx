...
import { bscKpiSummaryCardShellClass } from '@/app/(afterLogin)/(bsc)/bsc/_components/BscKpiMetricCard';
...
export default function MyScorecardKpiDetailPage() {
  ...
  const cardsWithKpi = useMemo(
    () =>
      mine.filter((card) => card.targets.some((t) => t.kpiLibraryId === kpiId)),
    [mine, kpiId],
  );
  ...
  const seriesAnchor = useMemo(() => {
    if (seriesAnchorId) {
      ...
      if (opt) {
        return mine.find((c) => c.id === opt.scorecardId) || entryScorecard;
      }
    }
  ...
  const isIndividualKpi = useMemo(() => {
    return cardsWithKpi.some((card) =>
      card.targets.some(
        (t) => t.kpiLibraryId === kpiId && t.assignmentSource === 'individual',
      ),
    );
  }, [cardsWithKpi, kpiId]);
  ...
  const periodRows = useMemo((): PeriodRow[] => {
    ...
    for (const card of seriesCards) {
      ...
      const target = card.targets.find((t) => t.kpiLibraryId === kpiId) || null;
  ...
  const columns: ColumnsType<PeriodRow> = [
    {
      ...
      key: 'period',
      render: (unused, row) => (
        <div data-cy="page-div-324" className="flex flex-col gap-0.5">
          <span data-cy="page-span-325" className={tableCellClassName}>
            {row.periodLabel}
          </span>
          <span data-cy="page-span-326" className="text-xs text-gray-500">
    ...
    {
      title: (
        <span data-cy="page-span-333" className={tableHeaderClassName}>
          Actual
        </span>
      ),
      key: 'actual',
      width: 110,
      render: (unused, row) => (
        <span data-cy="page-span-337" className={tableCellClassName}>
          {!row.assigned
            ? '—'
    ...
    {
      title: (
        <span data-cy="page-span-347" className={tableHeaderClassName}>
          Target
        </span>
      ),
      key: 'target',
      width: 110,
      render: (unused, row) => (
        <span data-cy="page-span-351" className={tableCellClassName}>
          {!row.assigned
            ? '—'
    ...
    {
      title: (
        <span data-cy="page-span-361" className={tableHeaderClassName}>
          Score
        </span>
      ),
      key: 'score',
      ...
      render: (unused, row) =>
        !row.assigned ? (
          <span
            ...
            data-cy={`bsc-my-kpi-unassigned-${row.key}`}
          >
            Not assigned
          </span>
        ) : (
    ...
    {
      title: (
        <span data-cy="page-span-380" className={tableHeaderClassName}>
          Status
        </span>
      ),
      key: 'status',
  ...
  const back = () => {
    ...
    router.push(scorecardTabHref('mine'));
  };
    ...
    <div className="w-full" data-cy="bsc-my-scorecard-kpi-detail-page">
      ...
      >
        ...
        {seriesOptions.length > 1 ? (
          ...
          <Tag
            className={blueTagClassName}
            data-cy="bsc-my-scorecard-kpi-series-tag"
          >
            {scorecardContextLabel(
              seriesAnchor,
      ...
      {loading ? (
        ...
        <div data-cy="page-div-458" className="flex justify-center py-10">
        ...
        >
          ...
          >
            <div
              className={bscKpiSummaryCardShellClass}
            ...
            >
              <BscScoreDonut
                ...
                sizeClassName="h-[90px] w-[90px] shrink-0"
                centerClassName="text-base font-bold text-gray-900"
              />
              <div
              ...
              >
                <h2
                  data-cy="page-h2-485"
                  className="mb-0 truncate text-base font-semibold leading-snug text-gray-900"
                >
                  {kpiName}
                </h2>
                <p
                ...
                >
                  {[perspective, targetLogicLabel(logic)]
                    .filter(Boolean)
                    .join(' · ')}
                </p>
                {isIndividualKpi ||
            ...
            <div
              className="w-full sm:w-[320px] sm:flex-none"
          ...
          >
            ...
            >
              ...
              <p data-cy="page-p-524" className="mb-3 text-xs text-gray-400">
            ...
            >
              ...
              <p data-cy="page-p-539" className="mb-3 text-xs text-gray-400">
          ...
          >
            <div
              data-cy="page-div-554"
              className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 sm:px-5"
            >
              <h3
                data-cy="page-h3-555"
