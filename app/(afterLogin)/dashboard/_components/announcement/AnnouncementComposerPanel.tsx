...
import { Avatar, Form, Popover, Select, Spin, Tag } from 'antd';
...
import { MdGroups, MdKeyboardArrowDown, MdTag } from 'react-icons/md';
...
const AnnouncementComposerPanel = ({
  ...
  useEffect(() => {
    if (!active) return;
    const nextSpaces = listCollaborationSpaces();
    setSpaces(nextSpaces);
    setMentionedUserIds([]);
    form.setFieldsValue({
      body: '',
      spaceId: nextSpaces[0]?.id,
    });
    notifyBodyActivity(false);
  }, [active, form]);
  ...
  const selectedSpace = spaces.find((space) => space.id === spaceIdValue);
      ...
      >
        <div className="mb-3 flex shrink-0 flex-wrap items-center gap-2">
          ...
          <Form.Item
            name="spaceId"
            className="!mb-0"
            rules={[{ required: true, message: 'Choose a space' }]}
          >
            <Select
              size="small"
              popupMatchSelectWidth={220}
              className="min-w-[140px] max-w-[200px] [&_.ant-select-selector]:!h-7 [&_.ant-select-selector]:!rounded-md [&_.ant-select-selection-item]:!flex [&_.ant-select-selection-item]:!items-center [&_.ant-select-selection-item]:!text-xs"
              ...
              optionRender={(option) => {
                const space = option.data.space as CollaborationSpace;
                return (
                  <span
                    className="flex items-center gap-2 py-0.5"
                    data-cy={`create-announcement-space-option-${space.id}`}
                  >
                    <span
        ...
        {/* Fills remaining height so the composer docks to the bottom and grows upward. */}
...
const CollaborationComposer = ({
  ...
  const applyFormat = (format: string, value: string | boolean = true) => {
    ...
    if (format === 'color') {
      quill.format('color', current.color ? false : '#1e3a5f');
      return;
    }
