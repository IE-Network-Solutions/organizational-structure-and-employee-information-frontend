...
const okrHeaderCardShellClass =
  'flex flex-col gap-4 h-[115px] min-w-0 shadow-none rounded-lg border border-[#D9D9D9] bg-white p-3';
...
const useOkrHeaderStats = () => {
  ...
  return { objectiveDashboard, isLoading, vpScore };
};

const useRecognitionCarousel = () => {
  ...
  const appreciationStats = [
    {
      ...
      ...getTrendMeta(
        monthOverMonthDifference?.feedbackReceived?.Engagement?.appreciations ||
          0,
      ),
    },
    {
  ...
  const reprimandStats = [
    {
      id: 'engagement',
      label: 'Engagement',
      value: personalRecognition?.feedbackReceived?.Engagement?.reprimands || 0,
      ...getTrendMeta(
...
export const AverageOkrKpiCard = () => {
  const { objectiveDashboard, isLoading } = useOkrHeaderStats();
  if (isLoading) {
    return (
...
export const CompanyOkrKpiCard = () => {
  const { objectiveDashboard, isLoading } = useOkrHeaderStats();
  if (isLoading) {
    return (
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col mt-3" data-cy="okr-card-company-okr-body">
        <div
...
export const AppreciationKpiCard = () => {
  const { isPersonalRecognitionLoading, appreciationStats, appreciationIndex } =
    useRecognitionCarousel();
  if (isPersonalRecognitionLoading) {
    return (
...
export const ReprimandKpiCard = () => {
  const { isPersonalRecognitionLoading, reprimandStats, reprimandIndex } =
    useRecognitionCarousel();
  if (isPersonalRecognitionLoading) {
    return <OkrHeaderStatCardSkeleton data-cy="okr-card-reprimand-skeleton" />;
  }
  return (
    <Card
      bordered={false}
    ...
    >
      ...
      <div className="mt-3 overflow-hidden" data-cy="okr-card-reprimand-body">
        <div
          className="flex transition-transform duration-500 ease-in-out"
          style={{
            width: `${reprimandStats.length * 100}%`,
...
export const VpScoreKpiCard = () => {
  const { vpScore, isLoading } = useOkrHeaderStats();
  const router = useRouter();
  if (isLoading) {
    return <OkrHeaderStatCardSkeleton data-cy="okr-card-vp-score-skeleton" />;
  }
  return (
    <Card
      bordered={false}
    ...
    >
      ...
      <div className="flex flex-col mt-3" data-cy="okr-card-vp-score-body">
        ...
        <div className=" flex gap-2 items-center" data-cy="okr-card-details">
          <Progress
            percent={
              (Number(vpScore?.score || 0) / Number(vpScore?.maxScore || 100)) *
              100
            }
            showInfo={false}
            strokeColor="#1f4fd8"
