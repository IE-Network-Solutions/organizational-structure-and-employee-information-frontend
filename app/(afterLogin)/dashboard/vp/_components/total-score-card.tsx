...
const TotalScoreCard = () => {
  ...
  const {  isLoading: isRefreshing, isRefetching } =
  ...
  return (
    <div className="rounded-lg border border-[#e6e6e6] bg-white px-5 py-4">
        ...
        <div>
          ...
          <div className="flex items-end gap-2 mt-2">
            {(isScoreLoading && !vpScore) || isRefreshing || isRefetching ? (
              ...
              <>
                <h2 className="text-[30px] leading-[30px] font-semibold text-[#1f1f1f]">
                  {score.toFixed(2)}%
                </h2>
                <span className="text-sm text-[#10B981] ">Achieved</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="mt-4 h-3.5 w-full overflow-hidden rounded-full bg-[#f0f0f0]">
        <div className="h-full w-full bg-[#3b82f6]">
          <div className="h-full bg-[#10b981]" style={{ width: `${achievedWidth}%` }} />
        </div>
      </div>

      <div className="flex items-center gap-6 text-xs text-[#8c8c8c] mt-2">
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#10b981]" />
          Achieved <span className="text-[#595959]">{score.toFixed(2)}%</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#3b82f6]" />
          Target <span className="text-[#595959]">{target}%</span>
        </div>
      </div>
    </div>
  );
