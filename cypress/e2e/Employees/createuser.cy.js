describe('Employees information testing', () => {
    ...
    it('2.should fill out the employee form', () => {
      ...
      cy.get('#dependencies_profileImage', { timeout: 10000 })
        .attachFile(fileName1, { force: true });

        cy.get('#dependencies_userFirstName').type('a');
        cy.get('[id="dependencies_userFirstName_help"]').should('contain.text', 'name must be between 3 and 20 characters long.');
       cy.get('#dependencies_userFirstName').type('abrha');

        cy.get('#dependencies_userMiddleName').type('ka');
        cy.get('[id="dependencies_userMiddleName_help"]').should('contain.text', 'Middle Name must be between 3 and 20 characters long.');
        cy.get('#dependencies_userMiddleName').type('kahsay');  

        cy.get('#dependencies_userLastName').type('ef');
        cy.get('[id="dependencies_userLastName_help"]').should('contain.text', 'Last Name must be between 3 and 20 characters long.');
        cy.get('#dependencies_userLastName').type('efrem');
     
      
      cy.get('#dependencies_userEmail').type('abirha182gmail.com');
      cy.get('[ id="dependencies_userEmail_help"]').should('contain.text', 'Invalid email format. Email must contain only one dot after @.');
     ...
     cy.get('#dependencies_employeeGender').click();
      ...
      cy.get('[id="dependencies_address_addressCountry_help"]').should('contain.text', 'country must be between 3 and 20 characters long.');
      cy.get('#dependencies_address_addressCountry').type('Ethiopia');

      cy.get('#dependencies_address_addressCity').type('A');
      cy.get('[id="dependencies_address_addressCity_help"]').should('contain.text', 'city must be between 3 and 20 characters long.');
      ...
      cy.get('.ant-select-item-option-content').contains('Input').click({ force: true });
    ...
    cy.wait(1000); 
cy.get('#dependencies_address_newinput8').should('be.visible').type('Test input');
    ...
    //   });
     ...
     cy.get('[id="dependencies_emergencyContact_emergencyContactFullName_help"]').should('contain.text', 'Full Name must be between 3 and 20 characters long.');
      cy.get('#dependencies_emergencyContact_emergencyContactFullName').type('abrkah');

      cy.get('#dependencies_emergencyContact_emergencyContactLastName').type('kA');
      cy.get('[id="dependencies_emergencyContact_emergencyContactLastName_help"]').should('contain.text', 'Last Name must be between 3 and 20 characters long.');
      cy.get('#dependencies_emergencyContact_emergencyContactLastName').type('kahabr');

      cy.get('#dependencies_emergencyContact_emergencyContactEmailAddress').type('abirha182gmail.com');
      cy.get('[id="dependencies_emergencyContact_emergencyContactEmailAddress_help"]').should('contain.text', 'Invalid email format. Email must contain only one dot after @.');
      cy.get('#dependencies_emergencyContact_emergencyContactEmailAddress').type('abirha182@gmail.com');
  
      cy.get('#dependencies_emergencyContact_emergencyContactGender').click({force:true});
      cy.get('.ant-select-item-option-content').contains('Male').click({force:true});
      cy.get('#dependencies_emergencyContact_emergencyContactDateOfBirth').type('2024-08-15');
  
      cy.get('#dependencies_emergencyContact_emergencyContactNationality').click();
      cy.get('.ant-select-item-option-content').contains('Ethiopian').click({force:true});
    ...
    // cy.get('.ant-select-item-option-content').contains('option 2').click({force:true});
      ...
      cy.get('[id="dependencies_bankInformation_bankName_help"]').should('contain.text', 'Bank Name must be between 3 and 20 characters long.');
     cy.get('#dependencies_bankInformation_bankName').type('Abiysinia')

      cy.get('#dependencies_bankInformation_branch').type('b');
      cy.get('[ id="dependencies_bankInformation_branch_help"]').should('contain.text', 'Branch must be between 3 and 20 characters long.');
      cy.get('#dependencies_bankInformation_branch').type('bole');

      cy.get('[id="dependencies_bankInformation_accountName"]').type('aC');
      cy.get('[id="dependencies_bankInformation_accountName_help"]').should('contain.text', 'Account Name must be between 3 and 20 characters long.');
    ...
    // cy.get('.ant-select-item-option-content').contains('option 2').click({force:true});
        ...
        cy.get('[id="dependencies_jobTitle_help"]').should('contain.text', 'job title must be between 3 and 20 characters long.');
        ...
        cy.get('.ant-select-item-option-content').contains('Software Solutions').click();
        ...
        cy.get('.ant-select-item-option-content').contains('admin role for tenant id 1').click();
      ...
      cy.get('#cancelSidebarButtonId').click({ force: true });
        ...
        cy.get('#cancelSidebarButtonId').click({ force: true });
            ...
            it('should display validation errors for required fields', () => {
                 ...
                 cy.get('#sidebarActionSubmitAndContinue1').click({ force: true });
                 cy.contains('button', 'Submit').click({force:true});
                 cy.get('#dependencies_documentName_help').should('contain.text', 'Please choose the document type');
                ...
                cy.get('#cancelSidebarButtonId').click({ force: true });
                 cy.get('[id="dependencies_joinedDate_help"]').should('contain.text', 'Please select the joined date');
                
                 cy.get('[id="dependencies_jobTitle_help"]').should('contain.text', 'job title is required.');
                
               cy.get('[id="dependencies_departmentId_help"]').should('contain.text', 'Please select a department');
                
               
               cy.get('[id="dependencies_roleId_help"]').should('contain.text', 'Please select a role!');
             
               cy.get('[id="dependencies_setOfPermission_help"]').should('contain.text', 'Please select at least one permission!');
                
               cy.get('[id="dependencies_workScheduleId_help"]').should('contain.text', 'Please select a work schedule!');
               cy.get('#cancelSidebarButtonId').click({ force: true });
               


               cy.get('[id="dependencies_profileImage_help"]').should('contain.text', 'Please upload your profile image!');
               
               cy.get('[id="dependencies_userFirstName_help"]').should('contain.text', 'name is required.');
               
               cy.get('[id="dependencies_userMiddleName_help"]').should('contain.text', 'Middle Name is required.');
               
               cy.get('[id="dependencies_userLastName_help"]').should('contain.text', 'Last Name is required.');
               
               cy.get('[ id="dependencies_userEmail_help"]').should('contain.text', 'Email is required.');
              
               cy.get('[ id="dependencies_employeeGender_help"]').should('contain.text', 'Please enter Gender');
              
               cy.get('[id="dependencies_dateOfBirth_help"]').should('contain.text', 'Please enter Date of Birth');
              
               cy.get('[id="dependencies_nationalityId_help"]').should('contain.text', 'Please enter Nationality');
               
               cy.get('[id="dependencies_martialStatus_help"]').should('contain.text', 'Please select a marital status!');
               
               cy.get('[id="dependencies_address_addressCountry_help"]').should('contain.text', 'country is required.');
               
               cy.get('[id="dependencies_address_addressCity_help"]').should('contain.text', 'city is required.');
              ...
              //  cy.get('[ id="dependencies_address_Account Type_help"]').should('contain.text', 'Account Type is required');
               ...
               cy.get('[id="dependencies_emergencyContact_emergencyContactFullName_help"]').should('contain.text', 'Full Name is required.');
               
               cy.get('[id="dependencies_emergencyContact_emergencyContactLastName_help"]').should('contain.text', 'Last Name is required.');
            
               cy.get('[id="dependencies_emergencyContact_emergencyContactEmailAddress_help"]').should('contain.text', 'Email is required.');
              
               cy.get('[ id="dependencies_emergencyContact_emergencyContactGender_help"]').should('contain.text', 'Please enter Gender');
               
               cy.get('[id="dependencies_emergencyContact_emergencyContactDateOfBirth_help"]').should('contain.text', 'Please enter Date of Birth');
               
               cy.get('[id="dependencies_emergencyContact_emergencyContactNationality_help"]').should('contain.text', 'Please enter Nationality');
               
            

               
               cy.get('[id="dependencies_bankInformation_bankName_help"]').should('contain.text', 'Bank Name is required.');
              
               cy.get('[ id="dependencies_bankInformation_branch_help"]').should('contain.text', 'Branch is required.');
               
               cy.get('[id="dependencies_bankInformation_accountName_help"]').should('contain.text', 'Account Name is required.');
               
               cy.get('[id="dependencies_bankInformation_accountNumber_help"]').should('contain.text', 'Please enter Account Number');
              });
