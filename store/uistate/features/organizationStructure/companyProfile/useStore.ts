...
export const useCompanyProfile = create<CompanyInformation>((set) => ({
    ...
    companyName: '',
    companyDomainName: '',
    setCompanyProfile: (fileList: UploadFile<any> | undefined) => set({ companyProfileImage: fileList }),
    setCompanyName: (name: string) => set({ companyName: name }),
    setCompanyDomainName: (domainName: string) => set({ companyDomainName: domainName }),
  }));