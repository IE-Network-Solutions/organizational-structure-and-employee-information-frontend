    ...
    () => getAllCalculatedVpScore(userId),
    {
      ...
      onSuccess: async (emp) => {
        ...
        const latestDataArray = tempItemArray.map((td: any) => ({
          ...td,
          VpInPercentile: emp.find((employee: any) => employee.userId === td.name)?.vpInPercentile || td.VpInPercentile,
          VpInBirr: emp.find((employee: any) => employee.userId === td.name)?.vpInBirr || td.VpInBirr,
          Benefit: emp.find((employee: any) => employee.userId === td.name)?.benefit || td.Benefit,
          VpScore: emp.find((employee: any) => employee.userId === td.name)?.vpScore || td.VpScore,
        }));
