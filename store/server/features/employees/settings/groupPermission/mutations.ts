...
import { UpdatePermissionGroupArgs } from "./interface";
...
const createPermissionGroup = async (values: GroupPermissionkey,) => {
  ...
  const updatePermissionGroup = async (values: any) => {
    return crudRequest({url:`${ORG_AND_EMP_URL}/permission-group/${values?.permissionGroupId}` ,  method:"patch" , data:values})
   };
/**
