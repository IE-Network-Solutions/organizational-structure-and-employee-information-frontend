...
const CopilotIcon: React.FC<{ size?: number }> = ({ size = 20 }) => {
  ...
  return (
    <svg
      ...
      data-cy="copilot-icon"
    >
      <defs data-cy="copilot-icon-defs">
        <linearGradient
          id={gradientId}
          x1="0%"
          y1="0%"
          x2="100%"
          y2="100%"
          data-cy="copilot-icon-gradient"
        >
          <stop
            offset="0%"
            stopColor="#3B82F6"
            data-cy="copilot-icon-gradient-stop-1"
          />{' '}
          {/* Blue */}
          <stop
            offset="20%"
            stopColor="#06B6D4"
            data-cy="copilot-icon-gradient-stop-2"
          />{' '}
          {/* Teal/Cyan */}
          <stop
            offset="40%"
            stopColor="#10B981"
            data-cy="copilot-icon-gradient-stop-3"
          />{' '}
          {/* Green */}
          <stop
            offset="60%"
            stopColor="#F59E0B"
            data-cy="copilot-icon-gradient-stop-4"
          />{' '}
          {/* Yellow/Orange */}
          <stop
            offset="80%"
            stopColor="#F97316"
            data-cy="copilot-icon-gradient-stop-5"
          />{' '}
          {/* Orange */}
          <stop
            offset="100%"
            stopColor="#EC4899"
            data-cy="copilot-icon-gradient-stop-6"
          />{' '}
          {/* Pink */}
        </linearGradient>
      </defs>
