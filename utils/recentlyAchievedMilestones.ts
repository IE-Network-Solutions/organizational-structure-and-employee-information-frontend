...
function normalizeReportStatus(raw: unknown): string {
  return String(raw ?? '')
    .trim()
    .toLowerCase()
    .replace(/[\s-]+/g, '_');
...
function isDoneReportStatus(raw: unknown): boolean {
  const status = normalizeReportStatus(raw);
  return (
    status === 'done' ||
    status === 'completed' ||
    status === 'complete' ||
...
function taskMilestoneId(task: any): string | null {
  const mid =
    ...
    task?.planTask?.outcomeMilestoneId ??
    null;
...
/**
 * All achieveMK / milestone-outcome IDs present on a report form structure
 * (regardless of Done/Not). Used to reopen sticky disables on cancel/edit.
 */
export function collectLinkedMilestoneIdsFromReport(
  ...
  eachKeyResult(formattedData, (keyresult) => {
    ...
    (keyresult?.milestones ?? []).forEach((milestone: any) => {
      ...
      const hasAchieveMk = (milestone?.tasks ?? []).some((task: any) =>
        isAchieveMkTask(task),
      );
      // Only milestones that have an achieveMK outcome task on this report.
      if (mid && hasAchieveMk) ids.add(mid);
      (milestone?.tasks ?? []).forEach((task: any) => {
        if (!isAchieveMkTask(task)) return;
        const taskMid = taskMilestoneId(task) ?? mid;
        if (taskMid) ids.add(taskMid);
      });
    });
  });
...
/**
 * Previous achieved IDs for Done→Not: form/API statuses plus any linked
 * milestones still sticky-disabled in the session store for this report.
 */
export function mergePreviousAchievedWithSession(
  formattedData: any[] | null | undefined,
  previousFromStatuses: string[],
): string[] {
  const linked = collectLinkedMilestoneIdsFromReport(formattedData);
  const fromSession = linked.filter((id) => isRecentlyAchievedMilestone(id));
  return Array.from(new Set([...previousFromStatuses, ...fromSession]));
}
...
export function collectAchievedMilestoneIdsFromReport(
  ...
  const isDone = (task: any) => {
    ...
    return isDoneReportStatus(
      resolveTaskReportStatus(taskId, selectedStatuses, formValues),
    );
  };
